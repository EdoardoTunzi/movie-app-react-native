      ...
      <FlatList
        ...
        ListHeaderComponent={
          <>
            ...
            {loading && <ActivityIndicator size="large" color="#0000ff" className="my-3 self-center" />}
            {error && <Text className="text-red-500 px-5 my-3">Error: {error?.message}</Text>}
            {!loading && !error && searchQuery.trim() && movies?.length > 0 && (
              <Text className="text-xl text-white font-bold">
                {movies.length} Search results for{` `}
        ...
        ListEmptyComponent={
          !loading && !error ? (
            <View className="mt-10 px-5">
              <Text className="text-center text-lg text-white font-bold">{searchQuery ? "No results found" : "Search for a movie"}</Text>
            </View>
          ) : null
        }
      />
